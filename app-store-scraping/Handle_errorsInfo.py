...
def get_error_logs_json(original_path,logger,target_path):
    with open(original_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            if not json_match:
                print(f"can not find JSON, skip: {json_str}")
...
def filter_data_appstore_label(file_a,file_b,output_file,missing_ids_file):
    ...
    print(f"Missing ID path {missing_ids_file}")
...
def get_donot_have_google_label():
    GetCommon.read_id(r"pythonProject\Logger\GooglePlayLabel_error.log",r"pythonProject\AppInfo_google\a.json",r'[?&]id=([^&]+)')
    GetCommon.get_file_name(r"C:\Users\smile\PycharmProjects\pythonProject\Label_google",r"C:\Users\smile\PycharmProjects\pythonProject\AppInfo_google\a.json",3,"size","")
    app_info_datas = GetCommon.get_app_info(r"pythonProject\AppInfo_google\a.json")
    unique_data = list(set(app_info_datas))
    print(f"unique_data:{len(unique_data)}")
    GetCommon.save_new_json_file(r"pythonProject\AppInfo_google\donot_have_label.json",unique_data)

def get_donot_have_google_privacy():
    # app_info_datas = GetCommon.get_app_info(r"pythonProject\AppInfo_google\appInfo_google.json")
    app_info_datas = GetCommon.get_app_info(r"pythonProject\AppInfo_google\not_have_privacy_loop2_1.json")
    driver = GetCommon.setup_driver()  # Or any other browser

    for app_id in app_info_datas:
        ...
        if a_tags:
            ...
            GetCommon.save_add_json_file(r"pythonProject\AppInfo_google\have_privacy.json", json_data)
        else:
            GetCommon.save_add_json_file(r"pythonProject\AppInfo_google\not_have_privacy.json", app_id)
...
if __name__ == "__main__":
    ...
    # get_donot_have_google_label()

    get_donot_have_google_privacy()
